...
class Integration_I18n_Test extends \Codeception\TestCase\WPTestCase {
	...
	public function test_load_plugin_textdomain(): void {
		...
		self::assertTrue( file_exists( $plugin_root_dir . '/languages/' ), '/languages/ folder does not exist.' );

		// Seems to fail because there are no translations to load.
		self::assertTrue( is_textdomain_loaded( 'bh-wp-simple-calendar' ), 'i18n text domain not loaded.' );
	}
}
